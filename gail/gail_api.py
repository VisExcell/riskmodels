...
class GailAPI:
    def __init__(self):
        self.name = "GAIL"
        self.description = "Calculator based off GAIL"
        self.fields = {
            "age": {
                "description": "Patient Current Age between 35 and 85",
                "type": "range",
                "maximum": 85,
                "minimum": 35},
            "num_biopsy": {
                "description": "How many breast biopsies has the patient had?",
                "type": "enum",
                "values": [0, 1, 2],
            ...
            "menarch_age": {
                "description": "Patient's age at the time of her first menstrual period?",
                "type": "enum",
                "values": [0, 1, 2],
            ...
            "race": {
                "description": "What is the patient's race/ethnicity?",
                "type": "enum",
                # Note "input_mappings" are not used at the moment.
    ...
    def validate_inputs(self, inputs):
        ...
        for k in valid:
            if self.fields[k]["type"] is "enum":
                # print "Validating field %s" % k
                if inputs[k] not in self.fields[k]["values"]:  # ["input_mappings"].keys():
                    out_of_bounds.append(k)
            elif self.fields[k]["type"] is "range":
                # print "validating field %s with value %d" %(k,inputs[k])
    ...
    def run(self, inputs):
        ...
        # print return_obj
        return return_obj  # json.dumps(return_obj)
